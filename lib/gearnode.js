...
function Gearnode(){
    ...
    this.workerId = null;
    this.distributionMode = this.LAST; //default 
    this.cycleCount = 0;
}
...
/**
 * Gearnode#setDistributionMode -> undefined
 * - mode [LAST|FIRST|RANDOM]
 *
 * sets the mode in which jobs are distributed across servers
 **/
Gearnode.prototype.setDistributionMode = function(mode) {
  if( mode == this.FIRST
  || mode == this.LAST
  || mode == this.RANDOM
  || mode == this.CYCLE
...
Gearnode.prototype.submitJob = function(func_name, payload, options){
    ...
    // default to LAST
    var serverIndex = this.server_names.length-1;
    if (this.distributionMode == this.FIRST) {
      serverIndex = 0;
    } else if (this.distributionMode == this.RANDOM) {
      serverIndex = Math.floor(Math.random() * this.server_names.length);
    } else if (this.distributionMode == this.CYCLE) {
      serverIndex = this.cycleCount % this.server_names.length;
      this.cycleCount = serverIndex + 1;
    }

    var server = this.servers[this.server_names[serverIndex]];
    console.log(server);
